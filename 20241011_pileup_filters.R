...
###################################################################################################################################
###################################################################################################################################
# FILTERING: LISTS OF FILTERS TO IMPLEMENT

# 1 QC (FILTER COLUMN: PASS VS OTHERWISE)
# 2 Mapped to chr Y 

# 3 COVERAGE
# No reads mapped at all in one or more samples
# Minimum coverage in any sample = 20
# Maximum coverage in any sample = 50

# 4 MTR (NR OF VARIANT READS)
# mutations with MTR >= 4 in all samples 
# mutations with MTR < 4 in all samples
# FLAG mutations with MTR >= 4 in one / all but one samples 

# 5 VAF
# Minimum VAF = 0.1 in one sample 

# 6 GERMLINE FILTER 
# 1-SIDED exact binomial on samples from both twins 
# FLAG putative germline in twin1 / twin2
# only use normal samples 

# 7 STRAND BIAS FILTER
# binomial w/ reads mapped to either strand
# only use normal samples 

# 8 MDR
# MDR = 3 in all samples (1)
# MDR = 3 in 10 / 12 normal samples (loss of chr1/18 in tumour)
...
paste('Fraction of DEP = MTR + WTR:', dim(twins_dt[PD62341ak_MTR+PD62341ak_WTR==PD62341ak_DEP])[1] / dim(twins_dt)[1])
# returns 1 so all good 
...
twins_dep[,sum_noreads := rowSums(.SD==0), .SDcols = cols_dep]
paste('Number of mutations w/o any reads in at least one sample:', dim(twins_dep[sum_noreads>0])[1]) # 3144
...
twins_dep[,f3_lowDepth_normal := as.numeric(min_dep_normal < 15)] # 1 if does not pass the filter 
twins_dep[,f3_highDepth_normal := as.numeric(max_dep_normal > 60)] # 1 if does not pass the filter
twins_dep[,f3_noReadsMapped := as.numeric(sum_noreads > 0)] # 1 if does not pass the filter
...
paste('Number of mutations identified in all samples (MTR >= 4):', dim(twins_mtr[sum_mtr4==length(samples)])[1]) # 322,563

# 3 identify mutations present in no samples (by MTR = 4 cut-off)
paste('Number of mutations identified in no samples (MTR >= 4):', dim(twins_mtr[sum_mtr4==0])[1]) # 5,310
...
twins_mtr[,f4_mtr0_absentInOne := as.numeric(sum_mtr0==1)] # no reads in one sample 
twins_mtr[,f4_mtr4_absentInOne_normal := as.numeric(sum_mtr4_normal==length(samples_normal)-1)] # should check if this is one sample or dist across all 
twins_mtr[,f4_mtr0_absentInOne_normal := as.numeric(sum_mtr0_normal==1)] # should check if this is one sample or dist across all

# we would expect sum mtr0 + mtr4 to be 18 if the mutation is absent (MTR=0) or present (MTR=4)
# note that even at these thresholds, it is hard to have 100% confidence that mutation is present / absent
twins_mtr[,sum_mtr04 := sum_mtr0 + sum_mtr4]
mut_pres_vs_abs = twins_mtr[(sum_mtr4 != 22 & sum_mtr0 != 22 & sum_mtr04==22), mut_ID] %>% unlist()
paste('Number of clear present / absent mutations:', length(mut_pres_vs_abs)) # 459 
# lots of those are in chr18 / chr1 - likely in the deleted regions, most checked on JBrowse look very good
...
twins_strands[, f6_strandBias := as.numeric(significance == 'significant')] 
paste('Number of mutations which show strand bias:', dim(twins_strands[f6_strandBias==1])[1]) # 11,467
...
# fid mutations which are 3 (code for forward + reverse okay)
twins_mdr[, sum_MDR := rowSums(.SD==3), .SDcols = cols_mdr]
twins_mdr[, f6_MDR3all := as.numeric(sum_MDR!=12)] 
paste('Number of mutations with some strand bias:', dim(twins_mdr[f6_MDR3all==1])[1]) # 24,237

# perhaps let's say that we want '3' in >= 10 samples (that leaves 2 samples with strand bias so still should be okay)
twins_mdr[, f6_MDR3min10 := as.numeric(sum_MDR<10)] 
paste("Number of mutations with some strand bias in variant reads (in >= 10/12 samples):", dim(twins_mdr[f6_MDR3min10==1])[1]) # 15,290
...
twins_normal = twins_normal[sum_DEP_all!=0]
twins_normal = twins_normal[sum_DEP_PD62341!=0]
twins_normal = twins_normal[sum_DEP_PD63383!=0]
...
indel_pass = data.table(read.csv('Data/PD62341_PD63382_unmatched_pass_indels.txt', sep = '\t', header = FALSE))
indel_all = data.table(read.csv('Data/PD62341_PD63382_unmatched_indels_pass_or_fail.txt', sep = '\t', header = FALSE))

setnames(indel_pass, c('V1', 'V2', 'V3', 'V4'), c('Chrom', 'pos', 'ref', 'alt'))
setnames(indel_all, c('V1', 'V2', 'V3', 'V4'), c('Chrom', 'pos', 'ref', 'alt'))

# find indels that did not pass the threshold for filtering
indel_pass[,mut_ID := paste(Chrom, pos, ref, alt, sep = '_')]
indel_all[,mut_ID := paste(Chrom, pos, ref, alt, sep = '_')]

indel_pass_id = indel_pass[, mut_ID] %>% unlist()
indel_all_id = indel_all[, mut_ID] %>% unlist()

indel_fail = setdiff(indel_all_id, indel_pass_id)
paste("Number of indels that failed QC:", length(indel_fail)) # 2,675,168

# subset to get a dataframe for indels that failed QC
indel_fail_dt = indel_all[mut_ID %in% indel_fail]
indel_fail_dt[,mut_ID]

# exclude reads where reference not mapped
indel_fail_dt = indel_fail_dt[!is.na(ref)]
indel_fail_dt[, length := str_count(alt) - str_count(ref)]

indel_fail_dt[, start10 := (fcase( # add start and end coordinates depending on if you have insertion or deletion
  length <= 0, pos + length - 10,
  length > 0, pos - 10
))] 
indel_fail_dt[, end10 := (fcase( 
  length <= 0, pos + 10,
  length > 0, pos + length + 10
))]

indel_fail_dt = indel_fail_dt[, c('Chrom', 'start10', 'end10'), with=FALSE]
indel_fail_dt[, chrom_start := paste(Chrom, start10, sep = '_')]
indel_fail_dt[, chrom_end := paste(Chrom, end10, sep = '_')]
chrom_start = indel_fail_dt[, chrom_start] %>% unique() %>% unlist()
chrom_end = indel_fail_dt[, chrom_end] %>% unique() %>% unlist()
...
twins_dt[, f8_FailedIndelNearby := as.numeric(mut_ID %in% mut_indels)]
paste('Number of mutations near poor-quality indels:', dim(twins_dt[f8_FailedIndelNearby==1])[1]) # 7,952
...
mtr_filters = twins_mtr[, c('mut_ID', 'f4_mtr4_presentInAll', 'f4_mtr4_presentInNone', 'f4_mtr4_presentInOne', 'f4_mtr4_absentInOne', 'f4_mtr0_absentInOne', 'f4_mtr4_absentInOne_normal', 'f4_mtr0_absentInOne_normal'), with = FALSE]
vaf_filters = twins_vaf[, c('mut_ID', "f5_maxVAF01", "f5_VAFna") , with = FALSE]
strand_filters = twins_strands[, c('mut_ID', 'f6_strandBias'), with = FALSE]
mdr_filters = twins_mdr[, c('mut_ID', 'f6_MDR3all', 'f6_MDR3min10'), with = FALSE]
...
twins_dt_merge = merge(twins_dt_merge, mdr_filters, by = 'mut_ID')
twins_dt_filters = merge(twins_dt_merge, germline_filters, by = 'mut_ID')
...
dim(twins_dt_filters[f3_lowDepth_normal==1])[1] # 21,906
dim(twins_dt_filters[f3_highDepth==1])[1] # 1,432
dim(twins_dt_filters[f3_highDepth_normal==1])[1] # 19,038
...
dim(twins_dt_filters[f4_mtr4_absentInOne==1])[1] # 3,504
dim(twins_dt_filters[f4_mtr0_absentInOne==1])[1] # 10,389
dim(twins_dt_filters[f5_maxVAF01==1])[1] # 1,274
dim(twins_dt_filters[f5_VAFna==1])[1] # 241
dim(twins_dt_filters[f6_strandBias==1])[1] # 11,467
dim(twins_dt_filters[f6_MDR3all==1])[1] # 23,710
dim(twins_dt_filters[f6_MDR3min10==1])[1] # 14,763
dim(twins_dt_filters[f7_likelyGermline_PD62341==1])[1] # 352,685
dim(twins_dt_filters[f7_likelyGermline_PD63383==1])[1] # 352,881
dim(twins_dt_filters[f7_likelyGermline_bothTwins==1])[1] # 350,300
dim(twins_dt_filters[f8_FailedIndelNearby==1])[1] # 7,932

######################################################################################################
# CHECK NR OF MUTATIONS THAT PASS SPECIFIC FILTERS
...
columns_all_filters = c('f1_PASS', 'f2_mappedY', 'f3_lowDepth', 'f3_lowDepth_normal', 'f3_highDepth', 'f3_highDepth_normal', 'f3_noReadsMapped', 
                       'f4_mtr4_presentInAll', 'f4_mtr4_presentInNone', 'f4_mtr4_presentInOne', 'f4_mtr4_absentInOne', 'f4_mtr0_absentInOne', 'f4_mtr0_absentInOne_normal', 'f4_mtr0_absentInOne_normal',
                       'f5_maxVAF01', 'f5_VAFna', 'f6_strandBias', 'f6_MDR3all', 'f6_MDR3min10',
                       'f7_likelyGermline_PD62341', 'f7_likelyGermline_PD63383', 'f7_likelyGermline_bothTwins', 'f8_FailedIndelNearby')
columns_req_filters = c('f2_mappedY', 'f3_lowDepth_normal', 'f3_highDepth_normal', 'f3_noReadsMapped',
                         'f4_mtr4_presentInAll', 'f4_mtr4_presentInNone', 
                         'f5_maxVAF01', 'f5_VAFna', 'f6_MDR3min10',
                         'f7_likelyGermline_bothTwins', 'f8_FailedIndelNearby')
...
paste('Number of mutations that pass all filters:', dim(twins_dt_filters[sum_all_filters==0])[1]) # 22  
paste('Number of mutations that pass required filters:', dim(twins_dt_filters[sum_req_filters==0])[1]) # 462  

######################################################################################################
# Choose filters which are most relevant to building the phylogeny 

columns_req_filters2 = c('f2_mappedY', 'f3_lowDepth', 'f3_highDepth', 'f3_noReadsMapped',
                        'f4_mtr4_presentInAll', 'f4_mtr4_presentInNone', 
                        'f5_maxVAF01', 'f5_VAFna','f7_likelyGermline_bothTwins', 'f8_FailedIndelNearby')
twins_dt_filters[, sum_req_filters2 := rowSums(.SD), .SDcols = columns_req_filters2] 
paste('Number of mutations that pass required filters:', dim(twins_dt_filters[sum_req_filters2==0])[1]) # 2559  

cols_info = c('VariantID', 'Chrom', 'Pos', 'Ref', 'Alt', 'Qual', 'Filter', 'Gene', 'Transcript', 'RNA', 'CDS',
              'Protein', 'Effect')
twins_filtered = twins_dt_filters[sum_req_filters2==0]
...
paste('Number of mutations that passed required filters:', length(mut_included)) # 2,559
write.table(mut_included, 'Data/mutations_include_20241018.txt', quote = FALSE, col.names = F, row.names = F)
