...
###################################################################################################################################
# Screen for possible germline predisposition  
germline_mutations = read.table(germline_mutations, 'Data/mutations_likely_germline_20241026.txt', quote = FALSE, col.names = F, row.names = F)

# Determine which of those occur in protein coding regions
cols_info = c('VariantID', 'Chrom', 'Pos', 'Ref', 'Alt', 'Qual', 'Filter', 'Gene', 'Transcript', 'RNA', 'CDS',
              'Protein', 'Effect')
twins_germline = twins_dt[mut_ID %in% germline_mutations, c('mut_ID', col_info)]
