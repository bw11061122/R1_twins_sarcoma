...
# Extra code to do the ABC simulations (tree-based)
...
twin_nodes = tree3_df %>% filter(branch == max(tree3_df$branch)) %>% dplyr::select(node) %>% unlist()
twin_nodes_sub = twin_nodes[1:8] # only sample from the a 8 nodes which were generated by one ICM cell 
twin1_nodes = sample(twin_nodes_sub, nr_twin1)
plot.phylo(tree3)
nodelabels(twin_nodes, twin_nodes, col= 'blue')  
plot.phylo(tree3)
nodelabels(twin1_nodes, twin1_nodes, col= 'purple')  
...
# calculate total mutation burden in cells from twin1

# calculate total mutation burden in cells from twin2

# shared mutations
# I think the logic here would be to find the MRCA and calculate the mut burden in this cell 
mrca_node = getMRCA(tree3, twin_nodes)
plot.phylo(tree3)
nodelabels(twin_nodes, twin_nodes, col = 'blue')
nodelabels(twin1_nodes, twin1_nodes, col = 'purple')
nodelabels(mrca_node, mrca_node, col = 'red')
