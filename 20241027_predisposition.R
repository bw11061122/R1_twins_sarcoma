...
cols_info = c('VariantID', 'Chrom', 'Pos', 'Ref', 'Alt', 'Qual', 'Filter', 'Gene', 'Transcript', 'RNA', 'CDS',
              'Protein', 'Effect')
twins_germline_info = twins_dt[Chrom != 'chrY' & mut_ID %in% germline_mutations, c('mut_ID', cols_info), with=FALSE]

# Protein coding genes affected 
# Okay at the moment this is way too many genes to reasonably find anything 
prime3 = twins_germline_info[Effect == '3prime_UTR_variant',Gene] %>% unlist() %>% unique()
prime5 = twins_germline_info[Effect == '5prime_UTR_variant',Gene] %>% unlist() %>% unique()
downstream = twins_germline_info[Effect == 'downstream',Gene] %>% unlist() %>% unique()
upstream = twins_germline_info[Effect == 'upstream',Gene] %>% unlist() %>% unique()
nonsense = twins_germline_info[Effect == 'nonsense',Gene] %>% unlist() %>% unique()
missense = twins_germline_info[Effect == 'missense',Gene] %>% unlist() %>% unique()
nc_variant = twins_germline_info[Effect == 'nc_variant',Gene] %>% unlist() %>% unique()
